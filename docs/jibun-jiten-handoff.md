...
- 次は **Phase D2（全画面デザイン適用・一括）**。指示書あり（docs/jibun-jiten-phaseD2-指示書.md）
...
1. **Phase D2 着手（全画面デザイン適用・Fable 5 推奨）**。指示書: docs/jibun-jiten-phaseD2-指示書.md ⚠️前提の home-mock.html がリポジトリ未収蔵（Chat側成果物・要受領）
2. **未解決: 例文164文の照読＝一般公開前の必須項目**（照読観点に**連濁・音変化チェック**を追加。試用しながら修正する方式で進行中）

### たまった微修正（まとめて調整する。個別に着手しない）
- よみ問題の対象字の強調が弱い（2026-07-19 Phase 3b検証時の指摘。文中の赤字だけでは目立たない）※Phase 3d時点で未対応のまま
