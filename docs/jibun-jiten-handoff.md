...
- 次は **Phase 3c（「なんでかな?」ふりかえり表示）**。指示書あり（docs/jibun-jiten-phase3c-指示書.md）
...
1. **Phase 3c 着手（「なんでかな?」ふりかえり表示・Fable 5 推奨）**。指示書: docs/jibun-jiten-phase3c-指示書.md
2. **未解決: 例文164文の照読＝一般公開前の必須項目**（照読は後回し・試用しながら修正する方式で進行中。sentences-check.html で随時指摘可）

### たまった微修正（まとめて調整する。個別に着手しない）
- よみ問題の対象字の強調が弱い（2026-07-19 Phase 3b検証時の指摘。文中の赤字だけでは目立たない）
