...
- 次は **Phase D1（署名要素「定着」の演出）**。指示書あり（docs/jibun-jiten-phase-d1-指示書.md）。調整は settle-test.html で回す
...
1. **Phase D1 着手（署名要素「定着」・Fable 5 推奨）**。指示書: docs/jibun-jiten-phase-d1-指示書.md。承認後は じぶん字典 → れんしゅう各画面 → ためす・結果 の順にデザイン言語を被せる（核から外へ）
