...
### 今どこ
- **Phase 1a 検収済み（2026-07-19 実機検証OK・右/左の筆順含む）**。スキーマは実データ80字で破綻なし → 残り946字への拡張は安全と判断
- 本番稼働中: 書き味プロトタイプ index.html(v0.2) ／ 80字ビューア viewer.html(v0.1) ／ data/kanji-g1.json（確定データ）
- 次は **Phase 1b（照合エンジンv1）**。指示書あり（docs/jibun-jiten-phase1b-指示書.md）
...
### 次の一手
0. ~~GitHub名義~~／~~タスク0~~／~~Phase 0~~（合格）／~~Phase 1a データ基盤~~（検収済み）
1. **Phase 1b 着手（照合エンジンv1・Fable 5 推奨）**。指示書: docs/jibun-jiten-phase1b-指示書.md
